...
package com.chengzhang.zebracrossing.app.zcrossing.camera;
...
public final class CameraManager {
    ...
    /**
     * Autofocus callbacks arrive here, and are dispatched to the Handler
     * which requested them.
     */
    private final AutoFocusCallback autoFocusCallback;

    /**
     * Initializes this static object with the Context of the calling
     * Activity.
     *
    ...
    private CameraManager(Context context) {
        ...
        // Cupcake, so we use the older
        // Camera.setPreviewCallback() on 1.5 and earlier. For Donut and
        // later, we need to use
        // the more efficient one shot callback, as the older one can
        // swamp the system and cause it
        // to run out of memory. We can't use SDK_INT because it was
        // introduced in the Donut SDK.
     ...
     * where to place the barcode. This target helps with alignment as well
     * as forces the user to hold the device far enough away to ensure the
    ...
    public Rect getFramingRectInPreview() {
        if (framingRectInPreview == null) {
            ...
            // screenResolution.x;
            // rect.top = rect.top * cameraResolution.y /
            // screenResolution.y;
    ...
    /**
     * Converts the result points from still resolution coordinates to
     * screen coordinates.
     *
     * @param points
     ...
     *         offset appropriately so they can be drawn in screen
     *         coordinates.
     */
    /*
	 ...
	 * (points[x].getX() + 0.5f); output[x].y = frame.top + (int)
	 * (points[x].getY() + 0.5f); } return output; }
	 */
        ...
        switch (previewFormat) {
            ...
            case PixelFormat.YCbCr_422_SP:
                return new PlanarYUVLuminanceSource(data, width, height, rect.left, rect.top, rect.width(),
                        rect.height());
            default:
                // The Samsung Moment incorrectly uses this variant
                // instead of the 'sp' version.
                // Fortunately, it too has all the Y data up front, so
                // we can read it.
